...
class EPD(framebuf.FrameBuffer):
    ...
    def __init__(self, spi, cs, dc, rst_pin, busy, landscape=True):
        ...
        self.width = 200 if landscape else 200
        self.height = 200 if landscape else 200
        self.max_line_bytes = self.width // 8
        self.max_column_bytes = self.height
    ...
    def Load_White(self):
        cmd("\x24")
        for i in range(0,self.max_column_bytes-1):
            for k in range(0,self.max_line_bytes-1):
                self._data(b"\xff")
